...
for sub_data in datasets:
    ...
    for image in tqdm(file_list):
        ...
        if "T1" in file:
            command = [f"{FSLDIR}/bin/bet", f'data/msseg-2008-{sub_data}-nii/{folder}/{file}', f'data/msseg-2008-{sub_data}-nii/{folder}/{file}_brain',"-R","-f","0.6","-g","0"]
    ...
    for image in tqdm(file_list):
        ...
        if "T2" in file or "FLAIR" in file:
            ...
            nib.save(final_img, f"data/msseg-2008-{sub_data}-nii/{folder}/{file}_brain.nii.gz")
    ...
    for image in tqdm(file_list):
        ...
        if "lesion" not in file:
            ...
            ants.image_write(image_n4, f"data/msseg-2008-{sub_data}-nii/{folder}/{file}_brain_bias_correction.nii.gz")
