...
from kornia.augmentation import RandomHorizontalFlip, RandomVerticalFlip, RandomAffine, AugmentationSequential, Normalize
import kornia as K 
import numpy as np
...
class Segmenter(pl.LightningModule):
    def __init__(self, model, class_weight, num_classes, 
                 learning_rate, factor_lr, patience_lr, batch_size_predict=16):
        super().__init__()
        self.model=model
    ...
    def predict_patches(self, images):
        """return the patches"""
        prediction = torch.zeros((images.size(0), self.num_classes, images.size(2), images.size(3)), device= self.device)
    ...
    def validation_step_isbi(self, batch, batch_idx):
        ...
        metrics = {"batch_score_val": np.sum([metrics_mask1["isbi_score"], metrics_mask2["isbi_score"]]),
    ...
    def validation_step_msseg2016(self, batch, batch_idx):
        ...
        metrics = {"batch_score_val": metrics_mask["isbi_score"], 
    ...
    def validation_step_msseg2008(self, batch, batch_idx):
        ...
        metrics = {"batch_score_val": metrics_mask1["score"], 
        ...
        if "mask2" in batch:
            ...
            metrics = {"batch_score_val": np.mean([metrics_mask1["score"], metrics_mask2["score"]]),
    ...
    def configure_optimizers(self):
        ...
        if cfg.TRAIN.TASK == "isbi":
            ...
            lr_schedulers = {"scheduler": scheduler, "monitor": "test_dice","strict": False,}
        else: # for mseeg2008
            lr_schedulers = {"scheduler": scheduler, "monitor": "val_score","strict": False,}
        # return [optimizer]
