...
class ActiveFocalContourLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        y_true = y_true.long()
        yTrueOnehot = torch.zeros(y_true.size(0), self.num_classes, y_true.size(2), y_true.size(3), device=self.device)
        yTrueOnehot = torch.scatter(yTrueOnehot, 1, y_true, 1)
        y_pred = torch.clamp(y_pred,min=1e-5,max=1-1e-5)

        active_focal = - yTrueOnehot * (1+(1-y_pred)**self.gamma) * torch.log(y_pred) \
