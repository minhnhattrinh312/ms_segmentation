...
class Predict2Folder():
    def __init__(self, model, list_file_flair, data_mean_std, checkpoint_list, \
        ...
        self.model = model
        self.segmenter = Segmenter(model, cfg.DATA.CLASS_WEIGHT, cfg.DATA.NUM_CLASS, 
    ...
    def predict_patches(self, images):
        ...
        if cfg.PREDICT.MODE == "3D":
            prediction = torch.zeros((images.size(0), self.num_class, images.size(2), images.size(3), images.size(4)), device= self.device)
        else:
            prediction = torch.zeros((images.size(0), self.num_class, images.size(2), images.size(3)), device= self.device)
    ...
    def predict_subject2d(self, path_flair):
        ...
        for transpose_view, convert_view in zip(cfg.DATA.ORIGIN2CUT, cfg.DATA.CUT2ORIGIN):
            ...
            for i in range(transposed_flair.shape[-1]):
                ...
                slice_inputs = np.stack([slices_t1, slices_flair, slices_t2, slices_pd], axis=-1)
            ...
            if cfg.PREDICT.ENSEMBLE:
                for checkpoint, weight_ckpt in zip(self.checkpoint_list, cfg.PREDICT.WEIGHTS):
                    self.segmenter = Segmenter(self.model, cfg.DATA.CLASS_WEIGHT, cfg.DATA.NUM_CLASS, 
                                        cfg.OPT.LEARNING_RATE, cfg.OPT.FACTOR_LR, cfg.OPT.PATIENCE_LR)
                    self.segmenter = Segmenter.load_from_checkpoint(checkpoint_path=checkpoint, model=self.model,
    ...
    def submit2folder(self):
        ...
        if not cfg.PREDICT.ENSEMBLE:
            ...
            for checkpoint in self.checkpoint_list:
                ...
                self.segmenter = Segmenter.load_from_checkpoint(checkpoint_path=checkpoint, \
                    model=self.model, class_weight=cfg.DATA.CLASS_WEIGHT, num_classes=cfg.DATA.NUM_CLASS, 
                    learning_rate=cfg.OPT.LEARNING_RATE,
                    factor_lr=cfg.OPT.FACTOR_LR, patience_lr=cfg.OPT.PATIENCE_LR)
                self.segmenter = self.segmenter.to(self.device)
                self.segmenter.eval()
...
if __name__ == "__main__":
    ...
    if not cfg.PREDICT.MASK_EXIST:
        list_file_flair = sorted(glob.glob("data/data_isbi_2015/testdata_website/*/preprocessed/*flair*"))
    else:
    ...
    # Load checkpoint
    # predict
    folder = Predict2Folder(model, list_file_flair, data_mean_std, checkpoint_list, \
                device, team_name="nhatvin", mask_exist=cfg.PREDICT.MASK_EXIST, \
                batch_size=cfg.PREDICT.BATCH_SIZE, make_zip=True, name_archive=name_zip, \
